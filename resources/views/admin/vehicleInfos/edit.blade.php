...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.vehicleInfo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vehicle-infos.update", [$vehicleInfo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('car') ? 'is-invalid' : '' }}" name="car_id" id="car_id" required>
                    @foreach($cars as $id => $entry)
                        <option value="{{ $id }}" {{ (old('car_id') ? old('car_id') : $vehicleInfo->car->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('car'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('fuel') ? 'is-invalid' : '' }}" name="fuel" id="fuel" required>
                    ...
                    @foreach(App\Models\VehicleInfo::FUEL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('fuel', $vehicleInfo->fuel) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('fuel'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('steeling') ? 'is-invalid' : '' }}" type="text" name="steeling" id="steeling" value="{{ old('steeling', $vehicleInfo->steeling) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('air_bag') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="air_bag" id="air_bag" value="1" {{ $vehicleInfo->air_bag || old('air_bag', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('transmission') ? 'is-invalid' : '' }}" name="transmission" id="transmission" required>
                    ...
                    @foreach(App\Models\VehicleInfo::TRANSMISSION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('transmission', $vehicleInfo->transmission) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('audio_input') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="audio_input" id="audio_input" value="1" {{ $vehicleInfo->audio_input || old('audio_input', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('bluetooth') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="bluetooth" id="bluetooth" value="1" {{ $vehicleInfo->bluetooth || old('bluetooth', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('heated_seats') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="heated_seats" id="heated_seats" value="1" {{ $vehicleInfo->heated_seats || old('heated_seats', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('fm_radio') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="fm_radio" id="fm_radio" value="1" {{ $vehicleInfo->fm_radio || old('fm_radio', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('usb_input') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="usb_input" id="usb_input" value="1" {{ $vehicleInfo->usb_input || old('usb_input', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('gps_navigation') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="gps_navigation" id="gps_navigation" value="1" {{ $vehicleInfo->gps_navigation || old('gps_navigation', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('sunroof') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="sunroof" id="sunroof" value="1" {{ $vehicleInfo->sunroof || old('sunroof', 0) === 1 ? 'checked' : '' }}>
