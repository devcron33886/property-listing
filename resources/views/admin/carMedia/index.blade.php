...
@can('car_medium_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.car-media.create') }}">
                ...
                <tbody>
                    @foreach($carMedia as $key => $carMedium)
                        <tr data-entry-id="{{ $carMedium->id }}">
                            ...
                            <td>
                                @can('car_medium_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.car-media.show', $carMedium->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('car_medium_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.car-media.edit', $carMedium->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('car_medium_delete')
                                    <form action="{{ route('admin.car-media.destroy', $carMedium->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('car_medium_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.car-media.massDestroy') }}",
