...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.car-media.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="car_video">{{ trans('cruds.carMedium.fields.car_video') }}</label>
                <input class="form-control {{ $errors->has('car_video') ? 'is-invalid' : '' }}" type="text" name="car_video" id="car_video" value="{{ old('car_video', '') }}">
