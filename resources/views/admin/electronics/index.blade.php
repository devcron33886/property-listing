...
@can('electronic_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.electronics.create') }}">
                ...
                <tbody>
                    @foreach($electronics as $key => $electronic)
                        <tr data-entry-id="{{ $electronic->id }}">
                            ...
                            <td>
                                @can('electronic_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.electronics.show', $electronic->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('electronic_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.electronics.edit', $electronic->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('electronic_delete')
                                    <form action="{{ route('admin.electronics.destroy', $electronic->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('electronic_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.electronics.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-Electronic:not(.ajaxTable)').DataTable({ buttons: dtButtons })
