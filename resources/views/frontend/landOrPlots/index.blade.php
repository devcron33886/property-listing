        ...
        <div class="col-md-12">
            @can('land_or_plot_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.land-or-plots.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $landOrPlot->id }}">
                                        ...
                                        <td>
                                            @can('land_or_plot_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.land-or-plots.show', $landOrPlot->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('land_or_plot_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.land-or-plots.edit', $landOrPlot->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('land_or_plot_delete')
                                                <form action="{{ route('frontend.land-or-plots.destroy', $landOrPlot->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('land_or_plot_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.land-or-plots.massDestroy') }}",
