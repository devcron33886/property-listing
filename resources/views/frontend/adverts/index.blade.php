        ...
        <div class="col-md-12">
            @can('advert_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.adverts.create') }}">
                            ...
                            <tbody>
                                @foreach($adverts as $key => $advert)
                                    <tr data-entry-id="{{ $advert->id }}">
                                        ...
                                        <td>
                                            @can('advert_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.adverts.show', $advert->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('advert_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.adverts.edit', $advert->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('advert_delete')
                                                <form action="{{ route('frontend.adverts.destroy', $advert->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('advert_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.adverts.massDestroy') }}",
