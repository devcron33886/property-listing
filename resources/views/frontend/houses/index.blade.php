        ...
        <div class="col-md-12">
            @can('house_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.houses.create') }}">
                            ...
                            <tbody>
                                @foreach($houses as $key => $house)
                                    <tr data-entry-id="{{ $house->id }}">
                                        ...
                                        <td>
                                            @can('house_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.houses.show', $house->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('house_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.houses.edit', $house->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('house_delete')
                                                <form action="{{ route('frontend.houses.destroy', $house->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('house_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.houses.massDestroy') }}",
