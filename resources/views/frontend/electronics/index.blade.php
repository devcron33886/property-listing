                            ...
                            <tbody>
                                @foreach($electronics as $key => $electronic)
                                    <tr data-entry-id="{{ $electronic->id }}">
                                        ...
                                        <td>
                                            @can('electronic_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.electronics.show', $electronic->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('electronic_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.electronics.edit', $electronic->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('electronic_delete')
                                                <form action="{{ route('frontend.electronics.destroy', $electronic->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
