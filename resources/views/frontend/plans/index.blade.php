                        ...
                        <table class=" table table-bordered table-striped table-hover datatable datatable-Plan">
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        {{ trans('cruds.plan.fields.plan_status') }}
                                    </th>
                                    <th>
                                        &nbsp;
                                    </th>
                            ...
                            <tbody>
                                @foreach($plans as $key => $plan)
                                    <tr data-entry-id="{{ $plan->id }}">
                                        ...
                                        <td>
                                            {{ App\Models\Plan::PLAN_STATUS_RADIO[$plan->plan_status] ?? '' }}
                                        </td>
                                        <td>
                                            @can('plan_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.plans.show', $plan->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('plan_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.plans.edit', $plan->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('plan_delete')
                                                <form action="{{ route('frontend.plans.destroy', $plan->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('plan_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.plans.massDestroy') }}",
